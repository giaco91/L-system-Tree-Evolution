...
# print(l_system.rule['X'])
# TI=Depth_specific_tree_interpreter(depth=3,plus_angles=[np.pi/8,np.pi/5,np.pi/5],minus_angles=[-np.pi/8,-np.pi/6,-np.pi/5],lengths=[1,1,1],cross_sections=[0.2,0.1,0.05],leaf_density=1)
# TI.modify_depth(3)
...
for u in range(1):
	...
	for g in range(n_gen):
		...
		for i in range(n_trees-1):
			...
			ls.general_mutation('X',p=0.05,immune={'[',']'},words={'[X]','[F]'},max_character=15)
		...
		selected_ls, selected_ti,max_score,best_branches=selection(ls_list,ti_list,n_sel=1)
		...
		w,dl=best_ls.evolution('X',best_ti.depth)
